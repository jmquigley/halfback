...
test('Test the put command direct function', (done) => {
	...
	scaffold
		...
		.go({verbose: true}, (err: Error, obj: Scaffold) => {
			...
			expect(obj instanceof Scaffold).toBe(true);
			expect(scaffold.history.length).toBe(3);
			expect(scaffold.history[0].startsWith(`sudo -E tee ${fixture.dir}`)).toBe(true);
			expect(scaffold.history[1].startsWith(`sudo -E chown -R root.root ${fixture.dir}`)).toBe(true);
			expect(scaffold.history[2].startsWith(`sudo -E chmod -R 755 ${fixture.dir}`)).toBe(true);
			done();
		});
});
...
test('Test the mkdir command direct function', (done) => {
	...
	scaffold
		...
		.go({verbose: true}, (err: Error, obj: Scaffold) => {
			...
			expect(obj instanceof Scaffold).toBe(true);
			expect(scaffold.history.length).toBe(3);
			expect(scaffold.history[0].startsWith(`sudo -E [ ! -d ${dir} ] && sudo mkdir -p ${dir}`)).toBe(true);
			expect(scaffold.history[1].startsWith(`sudo -E chown -R root.root ${fixture.dir}`)).toBe(true);
			expect(scaffold.history[2].startsWith(`sudo -E chmod -R 755 ${fixture.dir}`)).toBe(true);
			done();
		});
